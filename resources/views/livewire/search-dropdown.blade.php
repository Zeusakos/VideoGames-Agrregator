<div x-data="{isVisible:true}" class="relative" @click.away="isVisible = false" >
    <input
                ...
                @foreach($searchResults as $game)
                    <li class="border-b border-gray-700">
                        <a href="{{route('games.show',$game['slug'])}}"
                           class="block hover:bg-gray-6000 flex items-center transition ease-in-out duration-150 px-3 py-3"
                        @keydown.tab="isVisible=false"
                        @if($loop->last) @keydown.tab="isVisible=false" @endif
                        >

                            @if(isset($game['cover']) && isset($game['cover']['url']))
