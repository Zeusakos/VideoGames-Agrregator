...
@section('content')
    <div class="container mx-auto px-4">
        <div class="game-details border-b border-gray-800 pb-12 flex flex-col lg:flex-row">
            <div class="flex-none">
                <img src="{{ $game->getCoverUrl()}}" alt="gamecover"
                     class="hover:opacity-75 transition ease-in-out duration-150">
            </div>
            <div class="lg:ml-12 xl:mr-64">
                <h2 class="font-semibold text-4xl leading-tight mt-1">{{$game->getName()}}</h2>
                <div class="text-gray-400">
                    <span>
                        {{ implode(',', $game->getGenres()) }}
                    </span>
                    <span>
                        &middot;
                        {{ $game->getCompany()  }}
                ...
                <div class="flex flex-wrap items-center mt-8">
                    ...
                    <div class="flex items-center space-x-4 mt-4 sm:mt-0 sm:ml-12">
                        @if($game->getSocialLinks()->getWebsiteUrl())
                            <div class="w-8 h-8 bg-gray-800 rounded-full flex justify-center items-center">
                                <a href="{{$game->getSocialLinks()->getWebsiteUrl()}}" class="hover:text-gray-400">
                        ...
                        @endif
                        @if ($game->getSocialLinks()->getInstagramUrl())
                            <div class="w-8 h-8 bg-gray-800 rounded-full flex justify-center items-center">
                                <a href="{{$game->getSocialLinks()->getInstagramUrl()}}" class="hover:text-gray-400">
                        ...
                        @endif
                        @if ($game->getSocialLinks()->getTwitterUrl())
                            <div class="w-8 h-8 bg-gray-800 rounded-full flex justify-center items-center">
                                <a href="{{$game->getSocialLinks()->getTwitterUrl()}}" class="hover:text-gray-400">
                        ...
                        @endif
                        @if ($game->getSocialLinks()->getFacebookUrl())
                            <div class="w-8 h-8 bg-gray-800 rounded-full flex justify-center items-center">
                                <a href="{{$game->getSocialLinks()->getFacebookUrl()}}" class="hover:text-gray-400">
                ...
                <p class="mt-12">{{$game->getSummary()}}</p>
                ...
                <div class="mt-12" x-data="{isTrailerModalVisible: false}">
                    ...
                    <template x-if="isTrailerModalVisible">
                        <div
                            ...
                            <div class="container mx-auto lg:px-32 rounded-lg overflow-y-ayto">
            ...
            <div class="grid gird-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 mt-8">
                @foreach($game->getScreenshots() as $screenshot)
                    <div>
                        ...
                        image='{{$screenshot}}'
                        ">
                            <img src="{{ $screenshot }}" alt="gamecover"
                                 class="hover:opacity-75 transition ease-in-out duration-150">
                        </a>
                    </div>
                @endforeach
